...
%% added by libaohe, 2012-9-25
%%--------------------BEGIN-------------------------
-type resword_fun() :: fun((string()) -> boolean()).
%%---------------------END--------------------------
...
scan_atom(Cs0, St, Line, Col, Toks, Ncs0) ->
    case scan_name(Cs0, Ncs0) of
        ...
        {Wcs,Cs} ->
            case (St#erl_scan.resword_fun)(Wcs) of
                true -> 
                    tok2(Cs, St, Line, Col, Toks, Wcs, list_to_atom(Wcs));
                false ->
                    tok3(Cs, St, Line, Col, Toks, atom, Wcs, Wcs)
            end
    end.
...
scan_variable(Cs0, St, Line, Col, Toks, Ncs0) ->
    case scan_name(Cs0, Ncs0) of
        ...
        {Wcs,Cs} ->
            tok3(Cs, St, Line, Col, Toks, var, Wcs, Wcs)
    end.
%%----------------------END----------------------------
...
scan_qatom(Cs, St, Line, Col, Toks, {Wcs,Str,Line0,Col0,Uni0}) ->
    case scan_string0(Cs, St, Line, Col, $\', Str, Wcs, Uni0) of
        ...
        {error,Nline,Ncol,Nwcs,Ncs} ->
            ...
            scan_error({string,$\',Estr}, Line0, Col0, Nline, Ncol, Ncs);
        {Ncs,Nline,Ncol,Nstr,Nwcs,?NO_UNICODE} ->
%% modified by libaohe, 2012-9-26
%%-------------------------BEGIN-------------------------------------
            Attrs = attributes(Line0, Col0, St, Nstr),
            scan1(Ncs, St, Nline, Ncol, [{atom,Attrs,Nwcs}|Toks])     
%%--------------------------END--------------------------------------
    end.
...
%% modified by libaohe, 2012-9-25, atom -> string
%%---------------BEGIN------------------
-spec reserved_word(Rwd :: string()) -> boolean().
